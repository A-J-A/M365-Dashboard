...
public class PdfReportGenerator : IDocument
{
    ...
    private void AdminRolesPage(ColumnDescriptor col)
    {
        ...
        col.Item().Text("Entra ID Admin Role Assignments")
            .FontSize(28).FontColor(_primary);
        col.Item().Height(6);
        col.Item().Text(
        ...
        // Summary table — role name + active/eligible counts
        ...
        col.Item().Table(t =>
        {
            ...
            foreach (var h in new[] { "Role", "Total", "Active", "Eligible (PIM)" })
                t.Cell().Background(LightGray).Border(1).BorderColor(BorderCol)
                    .Padding(4).Text(h).FontSize(8).Bold();
            foreach (var role in data.Roles)
            {
                var countColor = role.RoleName == "Global Administrator" && role.MemberCount > 5 ? Crit : BodyText;
                t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
                    .Text(role.RoleName).FontSize(8);
                t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
                    .Text(role.MemberCount.ToString()).FontSize(8).FontColor(countColor);
                t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
                    .Text(role.ActiveCount.ToString()).FontSize(8);
                t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
                    .Text(role.EligibleCount > 0 ? role.EligibleCount.ToString() : "-").FontSize(8)
                    .FontColor(role.EligibleCount > 0 ? Warn : "#9CA3AF");
            }
        });
        col.Item().Height(20);
        ...
        foreach (var role in data.Roles)
        {
            ...
            col.Item().Table(t =>
            {
                ...
                foreach (var h in new[] { "Name", "UPN", "Assignment", "MFA", "Account" })
                    t.Cell().Background(LightGray).Border(1).BorderColor(BorderCol)
                        .Padding(4).Text(h).FontSize(8).Bold();
                foreach (var m in role.Members.OrderBy(m => m.PrincipalType).ThenBy(m => m.AssignmentType).ThenBy(m => m.DisplayName))
                {
                    ...
                    var assignColor = m.AssignmentType == "Eligible" ? Warn
                                   : m.AssignmentType == "Active + Eligible" ? "#6B46C1"
                                   : BodyText;
                    var nameColor   = isSp ? NonCompliant : BodyText;
                    t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
                        .Text(m.DisplayName ?? "-").FontSize(8).FontColor(nameColor);
                    t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
                        .Text(isSp ? "App / Service Principal" : m.UserPrincipalName ?? "-").FontSize(7).FontColor(nameColor);
                    t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
                        .Text(m.AssignmentType).FontSize(8).FontColor(assignColor);
                    t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
                        .Text(isSp ? "N/A" : m.IsMfaRegistered ? "Yes" : "No").FontSize(8)
                        .FontColor(isSp ? NonCompliant : m.IsMfaRegistered ? Compliant : Crit);
                    t.Cell().Border(1).BorderColor(BorderCol).Padding(4)
