...
public class ExecutiveReportService : IExecutiveReportService
{
    ...
    public async Task<ExecutiveReportData> GatherDataAsync()
    {
        ...
        tasks.Add(Task.Run(async () =>
        {
            try { reportData.AdminRoles = await GetAdminRolesAsync(); }
    ...
    private Azure.Core.TokenCredential GetAppCredential()
    {
        ...
        var certThumb    = _configuration["AzureAd:ClientCertificateThumbprint"];
        var certPfxB64   = _configuration["AzureAd:ClientCertificatePfx"];
        var clientSecret = _configuration["AzureAd:ClientSecret"];

        if (!string.IsNullOrEmpty(certThumb) && !string.IsNullOrEmpty(certPfxB64))
        {
            try
            {
                var pfxBytes = Convert.FromBase64String(certPfxB64);
            ...
            catch (Exception ex) { _logger.LogWarning(ex, "Certificate load failed, falling back to secret"); }
        }
    ...
    {
        // Delegates to the same implementation logic as ExecutiveReportController.GetAdminRolesAsync
        // using the shared app credential and raw REST calls for reliability.
        ...
        try
        {
            ...
            catch (Exception ex) { _logger.LogWarning(ex, "Could not fetch MFA details for admin roles"); }
            ...
            try
            {
                var url = "https://graph.microsoft.com/v1.0/users?$select=id,displayName,userPrincipalName,accountEnabled&$top=999";
                while (!string.IsNullOrEmpty(url))
                {
                    var resp = await http.GetStringAsync(url);
                    var doc  = System.Text.Json.JsonDocument.Parse(resp);
                    if (doc.RootElement.TryGetProperty("value", out var vals))
                        foreach (var u in vals.EnumerateArray())
                        {
                            var id  = u.TryGetProperty("id",                out var ip)  ? ip.GetString()  : null;
                            var dn  = u.TryGetProperty("displayName",       out var dp)  ? dp.GetString()  : null;
                            var upn = u.TryGetProperty("userPrincipalName", out var up)  ? up.GetString()  : null;
                            var ena = u.TryGetProperty("accountEnabled",    out var ep)  && ep.GetBoolean();
                            if (id != null) userCache[id] = (dn, upn, ena);
                        }
                    url = doc.RootElement.TryGetProperty("@odata.nextLink", out var nxt) ? nxt.GetString() ?? "" : "";
                }
            }
            ...
            try
            {
                var url = "https://graph.microsoft.com/v1.0/servicePrincipals?$select=id,displayName&$top=999";
                while (!string.IsNullOrEmpty(url))
                {
                    var resp = await http.GetStringAsync(url);
                    var doc  = System.Text.Json.JsonDocument.Parse(resp);
                    if (doc.RootElement.TryGetProperty("value", out var vals))
                        foreach (var sp in vals.EnumerateArray())
                        {
                            var id = sp.TryGetProperty("id",          out var ip) ? ip.GetString() : null;
                            var dn = sp.TryGetProperty("displayName", out var dp) ? dp.GetString() : null;
                            if (id != null) spCache[id] = dn ?? "Unknown App";
                        }
                    url = doc.RootElement.TryGetProperty("@odata.nextLink", out var nxt) ? nxt.GetString() ?? "" : "";
                }
            }
            catch (Exception ex) { _logger.LogWarning(ex, "Could not bulk-fetch SPs for admin roles"); }

            (string? Dn, string? Upn, bool Ena) ResolveUser(string id) =>
                userCache.TryGetValue(id, out var c) ? c : (null, null, false);

            async Task<IEnumerable<string>> ExpandPrincipal(string principalId)
            {
                var (_, upn, _) = ResolveUser(principalId);
                if (upn != null) return new[] { principalId };
                    ...
                    while (!string.IsNullOrEmpty(url))
                    {
                        ...
                        if (doc.RootElement.TryGetProperty("value", out var v))
                            foreach (var m in v.EnumerateArray())
                                if (m.TryGetProperty("id", out var mp) && mp.GetString() is string uid)
                                    ids.Add(uid);
                        url = doc.RootElement.TryGetProperty("@odata.nextLink", out var nxt) ? nxt.GetString() ?? "" : "";
                    }
                    return ids;
            ...
            try
            {
                var url = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments" +
                          "?$select=principalId,roleDefinitionId&$expand=roleDefinition($select=id,displayName)&$top=999";
                while (!string.IsNullOrEmpty(url))
                {
                    var resp = await http.GetStringAsync(url);
                    var doc  = System.Text.Json.JsonDocument.Parse(resp);
                    if (doc.RootElement.TryGetProperty("value", out var vals))
                        foreach (var a in vals.EnumerateArray())
                        {
                            var defId = a.TryGetProperty("roleDefinitionId", out var d) ? d.GetString() ?? "" : "";
                            var pid   = a.TryGetProperty("principalId",      out var p) ? p.GetString() ?? "" : "";
                            if (string.IsNullOrEmpty(defId) || string.IsNullOrEmpty(pid)) continue;
                            if (!activeByRole.ContainsKey(defId)) activeByRole[defId] = new(StringComparer.OrdinalIgnoreCase);
                            activeByRole[defId].Add(pid);
                            if (!roleNames.ContainsKey(defId) && a.TryGetProperty("roleDefinition", out var rd) && rd.TryGetProperty("displayName", out var dn))
            ...
            try
            {
                ...
                while (eligiblePage != null)
                {
                    foreach (var e in eligiblePage.Value ?? new())
                    {
                        ...
                        var pid   = e.PrincipalId ?? "";
                        if (string.IsNullOrEmpty(defId) || string.IsNullOrEmpty(pid)) continue;
                        if (!eligibleByRole.ContainsKey(defId)) eligibleByRole[defId] = new(StringComparer.OrdinalIgnoreCase);
                        eligibleByRole[defId].Add(pid);
            ...
            foreach (var roleId in allRoleIds.OrderBy(id => roleNames.TryGetValue(id, out var n) ? n : id))
            {
                ...
                var allPrincipals = new HashSet<string>(activeUsers, StringComparer.OrdinalIgnoreCase);
                foreach (var u in eligibleUsers) allPrincipals.Add(u);
                if (allPrincipals.Count == 0) continue;

                var roleEntry   = new AdminRoleEntry { RoleName = roleName };
                var addedToRole = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                var userActive  = new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase);
                var userEligible = new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase);

                foreach (var pid in activeUsers)   { foreach (var uid in await ExpandPrincipal(pid)) userActive[uid]   = true; }
                foreach (var pid in eligibleUsers) { foreach (var uid in await ExpandPrincipal(pid)) userEligible[uid] = true; }

                var resolved = new HashSet<string>(userActive.Keys, StringComparer.OrdinalIgnoreCase);
                foreach (var uid in userEligible.Keys) resolved.Add(uid);

                foreach (var userId in resolved)
                {
                    if (!addedToRole.Add(userId)) continue;
                    var (dn, upn, ena) = ResolveUser(userId);
                    var isAct  = userActive.ContainsKey(userId);
                    var isElig = userEligible.ContainsKey(userId);
                    var atype  = (isAct && isElig) ? "Active + Eligible" : isAct ? "Active" : "Eligible";

                    if (upn != null)
                    {
                        seenUserIds.Add(userId);
                        roleEntry.Members.Add(new AdminRoleMember { DisplayName = dn, UserPrincipalName = upn, AccountEnabled = ena, IsMfaRegistered = mfaRegisteredIds.Contains(userId), AssignmentType = atype, PrincipalType = "User" });
                    }
                    else if (spCache.TryGetValue(userId, out var spName))
                    {
                        roleEntry.Members.Add(new AdminRoleMember { DisplayName = spName, UserPrincipalName = "Service Principal", AccountEnabled = true, IsMfaRegistered = false, AssignmentType = atype, PrincipalType = "ServicePrincipal" });
                    }
                }
            ...
            result.GlobalAdminCount = result.Roles.FirstOrDefault(r => r.RoleName == "Global Administrator")?.MemberCount ?? 0;
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Error fetching admin roles"); }
        return result;
    }
}
