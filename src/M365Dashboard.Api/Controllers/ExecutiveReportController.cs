...
public class ExecutiveReportController : ControllerBase
{
    ...
    public async Task<IActionResult> DownloadReport([FromQuery] int month = 0, [FromQuery] int year = 0)
    {
        try
        {
            ...
            // Fetch sign-in locations and generate map image for PDF
            try
            {
                ...
                _logger.LogInformation("Sign-in locations: {Count} sign-ins returned", signIns?.Value?.Count ?? 0);

                var byCountry = new Dictionary<string, (int count, double lat, double lon)>(StringComparer.OrdinalIgnoreCase);
                foreach (var s in signIns?.Value ?? new())
                {
                ...
                reportData.SignInLocations = byCountry.Select(kvp => new SignInLocationData
                {
                ...
                _logger.LogInformation("Sign-in locations aggregated: {Count} countries", reportData.SignInLocations.Count);

                // Generate Azure Maps static image
                var mapsKey = _configuration["AzureMaps:SubscriptionKey"];
                _logger.LogInformation("AzureMaps key present: {HasKey}", !string.IsNullOrEmpty(mapsKey));

                if (!string.IsNullOrEmpty(mapsKey))
                {
                    var inv = System.Globalization.CultureInfo.InvariantCulture;
                    var validPins = reportData.SignInLocations
                    ...
                    if (validPins.Any())
                    {
                        ...
                        mapUrl += $"&pins=default|coFF6600||{coords}||";
                    }

                    _logger.LogInformation("Calling Azure Maps (pins: {Count})", validPins.Count);
                    using var http = new System.Net.Http.HttpClient { Timeout = TimeSpan.FromSeconds(20) };
                    var mapResp = await http.GetAsync(mapUrl);
                    if (mapResp.IsSuccessStatusCode)
                    {
                        reportData.SignInMapImageBytes = await mapResp.Content.ReadAsByteArrayAsync();
                        _logger.LogInformation("Azure Maps returned {Bytes} bytes", reportData.SignInMapImageBytes.Length);
                    }
                    else
                    {
                        var body = await mapResp.Content.ReadAsStringAsync();
                        _logger.LogWarning("Azure Maps returned {Status}: {Body}", mapResp.StatusCode, body);
                    }
                }
            }
    ...
    private async Task<AdminRolesData> GetAdminRolesAsync()
    {
        ...
        {
            // ── MFA lookup ───────────────────────────────────────────────────
            var mfaRegisteredIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            try
            {
                var mfaPage = await _graphClient.Reports.AuthenticationMethods.UserRegistrationDetails.GetAsync(config =>
                {
                    config.QueryParameters.Select = new[] { "id", "isMfaRegistered" };
                    config.QueryParameters.Top = 999;
                });
                while (mfaPage != null)
                {
                    foreach (var item in mfaPage.Value ?? new())
                        if (item.IsMfaRegistered == true && item.Id != null)
                            mfaRegisteredIds.Add(item.Id);
                    if (mfaPage.OdataNextLink == null) break;
                    mfaPage = await _graphClient.Reports.AuthenticationMethods.UserRegistrationDetails
                        .WithUrl(mfaPage.OdataNextLink).GetAsync();
                }
                _logger.LogInformation("MFA lookup built: {Count} MFA-registered users", mfaRegisteredIds.Count);
            }
            catch (Exception ex) { _logger.LogWarning(ex, "Could not fetch MFA registration details"); }

            // ── Shared HTTP client using app credentials — used for both user lookup and group expansion ──
            System.Net.Http.HttpClient? adminGraphClient = null;
            try
            {
                var appCred = GetAppCredential();
                var appToken = await appCred.GetTokenAsync(
                    new Azure.Core.TokenRequestContext(new[] { "https://graph.microsoft.com/.default" }), default);
                adminGraphClient = new System.Net.Http.HttpClient();
                adminGraphClient.DefaultRequestHeaders.Authorization =
                    new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", appToken.Token);
                adminGraphClient.DefaultRequestHeaders.Add("Accept", "application/json");
            }
            catch (Exception ex) { _logger.LogWarning(ex, "Could not create app-credential HTTP client for admin roles"); }

            // ── Bulk user lookup — fetch all users once, keyed by object ID ──────
            var userCache = new Dictionary<string, (string? DisplayName, string? Upn, bool Enabled)>(StringComparer.OrdinalIgnoreCase);
            // ── Bulk service principal lookup — keyed by object ID ──────────────
            var spCache = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase); // id → displayName

            if (adminGraphClient != null)
            {
                // Fetch all users
                try
                {
                    var usersUrl = "https://graph.microsoft.com/v1.0/users?$select=id,displayName,userPrincipalName,accountEnabled&$top=999";
                    while (!string.IsNullOrEmpty(usersUrl))
                    {
                        var resp = await adminGraphClient.GetStringAsync(usersUrl);
                        var doc  = System.Text.Json.JsonDocument.Parse(resp);
                        if (doc.RootElement.TryGetProperty("value", out var values))
                            foreach (var u in values.EnumerateArray())
                            {
                                var id  = u.TryGetProperty("id",                out var idp)  ? idp.GetString()  : null;
                                var dn  = u.TryGetProperty("displayName",       out var dnp)  ? dnp.GetString()  : null;
                                var upn = u.TryGetProperty("userPrincipalName", out var upnp) ? upnp.GetString() : null;
                                var ena = u.TryGetProperty("accountEnabled",    out var enap) && enap.GetBoolean();
                                if (id != null) userCache[id] = (dn, upn, ena);
                            }
                        usersUrl = doc.RootElement.TryGetProperty("@odata.nextLink", out var next)
                            ? next.GetString() ?? "" : "";
                    }
                    _logger.LogInformation("User cache built: {Count} users", userCache.Count);
                }
                catch (Exception ex) { _logger.LogWarning(ex, "Could not bulk-fetch users for admin roles"); }

                // Fetch all service principals
                try
                {
                    var spUrl = "https://graph.microsoft.com/v1.0/servicePrincipals?$select=id,displayName&$top=999";
                    while (!string.IsNullOrEmpty(spUrl))
                    {
                        var resp = await adminGraphClient.GetStringAsync(spUrl);
                        var doc  = System.Text.Json.JsonDocument.Parse(resp);
                        if (doc.RootElement.TryGetProperty("value", out var values))
                            foreach (var sp in values.EnumerateArray())
                            {
                                var id = sp.TryGetProperty("id",          out var idp) ? idp.GetString() : null;
                                var dn = sp.TryGetProperty("displayName", out var dnp) ? dnp.GetString() : null;
                                if (id != null) spCache[id] = dn ?? "Unknown App";
                            }
                        spUrl = doc.RootElement.TryGetProperty("@odata.nextLink", out var next)
                            ? next.GetString() ?? "" : "";
                    }
                    _logger.LogInformation("Service principal cache built: {Count} SPs", spCache.Count);
                }
                catch (Exception ex) { _logger.LogWarning(ex, "Could not bulk-fetch service principals for admin roles"); }
            }

            // Returns user info if known, or (null, null, false) if not a user.
            (string? DisplayName, string? Upn, bool Enabled) ResolveUser(string principalId) =>
                userCache.TryGetValue(principalId, out var cached) ? cached : (null, null, false);

            // Expands a principalId to the set of user IDs it represents.
            // If the principal is a user, returns {principalId}.
            // If the principal is a group, returns all transitive user members.
            // Service principals (apps) are silently ignored.
            async Task<IEnumerable<string>> ExpandPrincipalAsync(string principalId)
            {
                var (_, upn, _) = ResolveUser(principalId);
                if (upn != null) return new[] { principalId }; // it's a user

                // If it's a known service principal, return it directly — no group expansion needed
                if (spCache.ContainsKey(principalId)) return new[] { principalId };

                if (adminGraphClient == null) return Array.Empty<string>();

                // Try to expand as a group
                try
                {
                    var url = $"https://graph.microsoft.com/v1.0/groups/{principalId}/transitiveMembers/microsoft.graph.user?$select=id&$top=999";
                    var userIds = new List<string>();
                    while (!string.IsNullOrEmpty(url))
                    {
                        var resp = await adminGraphClient.GetStringAsync(url);
                        var doc  = System.Text.Json.JsonDocument.Parse(resp);
                        if (doc.RootElement.TryGetProperty("value", out var values))
                            foreach (var item in values.EnumerateArray())
                                if (item.TryGetProperty("id", out var idProp) && idProp.GetString() is string uid)
                                    userIds.Add(uid);
                        url = doc.RootElement.TryGetProperty("@odata.nextLink", out var next)
                            ? next.GetString() ?? "" : "";
                    }
                    _logger.LogInformation("Group {Id} expanded to {Count} users", principalId, userIds.Count);
                    return userIds;
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Could not expand principal {Id} as group", principalId);
                    return Array.Empty<string>();
                }
            }

            // ── Active assignments: ALL roles via raw REST to avoid SDK pagination/scope issues ─
            // roleDefinitionId→principalId mapping
            var activeByRole = new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);
            // roleDefinitionId→roleName mapping
            ...
            try
            {
                ...
                var token = await credential.GetTokenAsync(
                    new Azure.Core.TokenRequestContext(new[] { "https://graph.microsoft.com/.default" }), default);
                using var httpRoles = new System.Net.Http.HttpClient();
                httpRoles.DefaultRequestHeaders.Authorization =
                    new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token.Token);

                // Use raw REST with $top=999 and $expand to get all assignments in one pass
                // The SDK may silently cap results or miss certain scope types
                var url = "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments" +
                ...
                while (!string.IsNullOrEmpty(url))
                {
                    var resp = await httpRoles.GetStringAsync(url);
                    var doc = System.Text.Json.JsonDocument.Parse(resp);
                    ...
                    if (doc.RootElement.TryGetProperty("value", out var values))
                    {
                        ...
                        {
                            var defId      = a.TryGetProperty("roleDefinitionId", out var d) ? d.GetString() ?? "" : "";
                            var principalId = a.TryGetProperty("principalId",      out var p) ? p.GetString() ?? "" : "";
                            if (string.IsNullOrEmpty(defId) || string.IsNullOrEmpty(principalId)) continue;

                            if (!activeByRole.ContainsKey(defId))
                                activeByRole[defId] = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                            activeByRole[defId].Add(principalId);

                            // Capture role display name from expanded roleDefinition
                            if (!roleNames.ContainsKey(defId) &&
                                a.TryGetProperty("roleDefinition", out var rd) &&
                                rd.TryGetProperty("displayName", out var dn))
                                roleNames[defId] = dn.GetString() ?? defId;
                        }
                    }
                ...
                _logger.LogInformation("Active role assignments: {RoleCount} roles, {Total} total records. Roles: {Names}",
                    activeByRole.Count,
                    activeByRole.Values.Sum(v => v.Count),
                    string.Join(", ", roleNames.Values.OrderBy(n => n)));
            }
            catch (Exception ex) { _logger.LogWarning(ex, "Could not fetch active role assignments"); }

            // ── Eligible (PIM) assignments ────────────────────────────────────
            var eligibleByRole = new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase);
            try
            {
                var eligiblePage = await _graphClient.RoleManagement.Directory.RoleEligibilityScheduleInstances.GetAsync(config =>
                {
                    config.QueryParameters.Expand = new[] { "roleDefinition($select=id,displayName)" };
                    config.QueryParameters.Select = new[] { "principalId", "roleDefinitionId" };
                });
                while (eligiblePage != null)
                {
                    foreach (var e in eligiblePage.Value ?? new())
                    {
                        var defId = e.RoleDefinitionId ?? "";
                        var principalId = e.PrincipalId ?? "";
                        if (string.IsNullOrEmpty(defId) || string.IsNullOrEmpty(principalId)) continue;

                        if (!eligibleByRole.ContainsKey(defId)) eligibleByRole[defId] = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                        eligibleByRole[defId].Add(principalId);

                        if (!roleNames.ContainsKey(defId) && e.RoleDefinition?.DisplayName != null)
                            roleNames[defId] = e.RoleDefinition.DisplayName;
                    }
                    if (eligiblePage.OdataNextLink == null) break;
                    eligiblePage = await _graphClient.RoleManagement.Directory.RoleEligibilityScheduleInstances
                        .WithUrl(eligiblePage.OdataNextLink).GetAsync();
                }
                _logger.LogInformation("Eligible (PIM) role assignments: {RoleCount} roles", eligibleByRole.Count);
            }
            catch (Exception ex) { _logger.LogWarning(ex, "Could not fetch PIM eligible role assignments (may require P2 licence)"); }

            // ── Build combined role list ──────────────────────────────────────
            var allRoleIds = new HashSet<string>(activeByRole.Keys, StringComparer.OrdinalIgnoreCase);
            foreach (var k in eligibleByRole.Keys) allRoleIds.Add(k);

            // If we couldn't get role names from expansions, fetch role definitions
            var missingNames = allRoleIds.Where(id => !roleNames.ContainsKey(id)).ToList();
            if (missingNames.Any())
            {
                try
                {
                    var defs = await _graphClient.RoleManagement.Directory.RoleDefinitions.GetAsync(config =>
                        config.QueryParameters.Select = new[] { "id", "displayName" });
                    foreach (var d in defs?.Value ?? new())
                        if (d.Id != null && !roleNames.ContainsKey(d.Id))
                            roleNames[d.Id] = d.DisplayName ?? d.Id;
                }
                catch (Exception ex) { _logger.LogWarning(ex, "Could not fetch role definitions for name lookup"); }
            }

            var seenUserIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

            foreach (var roleId in allRoleIds.OrderBy(id => roleNames.TryGetValue(id, out var n) ? n : id))
            {
                var roleName = roleNames.TryGetValue(roleId, out var rn) ? rn : roleId;
                var activeUsers  = activeByRole.TryGetValue(roleId,   out var a) ? a : new HashSet<string>();
                var eligibleUsers = eligibleByRole.TryGetValue(roleId, out var e) ? e : new HashSet<string>();
                var allUsers = new HashSet<string>(activeUsers, StringComparer.OrdinalIgnoreCase);
                foreach (var u in eligibleUsers) allUsers.Add(u);

                if (allUsers.Count == 0) continue;

                var roleEntry = new AdminRoleEntry { RoleName = roleName };
                // Track resolved users for this role to avoid duplicates when a user
                // appears both directly and via a group assignment
                var usersAddedToRole = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

                // Expand each principal (may be a user or a group) into individual user IDs
                // Track which assignment type each user gets from each source
                var userActiveFlags   = new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase);
                var userEligibleFlags = new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase);

                foreach (var principalId in activeUsers)
                {
                    var expanded = await ExpandPrincipalAsync(principalId);
                    foreach (var uid in expanded)
                        userActiveFlags[uid] = true;
                }
                foreach (var principalId in eligibleUsers)
                {
                    var expanded = await ExpandPrincipalAsync(principalId);
                    foreach (var uid in expanded)
                        userEligibleFlags[uid] = true;
                }

                var allResolvedUsers = new HashSet<string>(userActiveFlags.Keys, StringComparer.OrdinalIgnoreCase);
                foreach (var uid in userEligibleFlags.Keys) allResolvedUsers.Add(uid);

                foreach (var userId in allResolvedUsers)
                {
                    if (!usersAddedToRole.Add(userId)) continue; // deduplicate

                    var (displayName, upn, enabled) = ResolveUser(userId);
                    var isActive   = userActiveFlags.ContainsKey(userId);
                    var isEligible = userEligibleFlags.ContainsKey(userId);
                    var assignmentType = (isActive && isEligible) ? "Active + Eligible"
                                       : isActive   ? "Active"
                                       : "Eligible";
                    ...
                    if (upn != null)
                    {
                        ...
                        roleEntry.Members.Add(new AdminRoleMember
                        {
                            DisplayName       = displayName,
                            UserPrincipalName = upn,
                            AccountEnabled    = enabled,
                            IsMfaRegistered   = mfaRegisteredIds.Contains(userId),
                            AssignmentType    = assignmentType,
                            PrincipalType     = "User",
                        });
                    }
                    else if (spCache.TryGetValue(userId, out var spName))
                    {
                    ...
                    // else: unknown/group already expanded — skip
                }

                roleEntry.MemberCount   = roleEntry.Members.Count;
                roleEntry.ActiveCount   = roleEntry.Members.Count(m => m.AssignmentType.Contains("Active"));
                roleEntry.EligibleCount = roleEntry.Members.Count(m => m.AssignmentType.Contains("Eligible"));

                if (roleEntry.MemberCount > 0)
                    result.Roles.Add(roleEntry);
            }
    ...
    private async Task<byte[]> GenerateWordDocument(ExecutiveReportData data, ReportSettings settings)
    {
        ...
        using (var wordDocument = WordprocessingDocument.Create(stream, WordprocessingDocumentType.Document, true))
        {
            ...
            {
                AddHeading(body, "Entra ID Admin Role Assignments", 2);
                var rolesSummaryTable = CreateTable(body, new[] { "Role", "Total", "Active", "Eligible" });
                foreach (var role in data.AdminRoles.Roles)
                    AddTableRow(rolesSummaryTable, new[]
                    {
                        role.RoleName,
                        role.MemberCount.ToString(),
                        role.ActiveCount.ToString(),
                        role.EligibleCount > 0 ? role.EligibleCount.ToString() : "-"
                    });
                ...
                foreach (var role in data.AdminRoles.Roles)
                {
                    ...
                    var memberTable = CreateTable(body, new[] { "Display Name", "UPN / Type", "Assignment", "MFA", "Account" });
                    foreach (var m in role.Members.OrderBy(m => m.PrincipalType).ThenBy(m => m.AssignmentType).ThenBy(m => m.DisplayName))
                    {
                        var isSp = m.PrincipalType == "ServicePrincipal";
                        AddTableRow(memberTable, new[]
                        {
                            m.DisplayName ?? "-",
                            isSp ? "App / Service Principal" : m.UserPrincipalName ?? "-",
                            m.AssignmentType,
                            isSp ? "N/A" : m.IsMfaRegistered ? "Yes" : "No",
...
<body>
    ...
    private string GenerateAdminRolesSection(AdminRolesData? data)
    {
        ...
        var sb = new System.Text.StringBuilder();
        sb.AppendLine("<div class='section'>");
        sb.AppendLine("<h2>Entra ID Admin Role Assignments</h2>");
        ...
        // Summary table
        sb.AppendLine("<table>");
        sb.AppendLine("<tr><th>Role</th><th>Total</th><th>Active</th><th>Eligible (PIM)</th></tr>");
        foreach (var role in data.Roles)
        {
            var countColor = role.RoleName == "Global Administrator" && role.MemberCount > 5 ? " class='critical'" : "";
            sb.AppendLine($"<tr>" +
                $"<td>{role.RoleName}</td>" +
                $"<td{countColor}>{role.MemberCount}</td>" +
                $"<td>{role.ActiveCount}</td>" +
                $"<td>{(role.EligibleCount > 0 ? role.EligibleCount.ToString() : "-")}</td>" +
                $"</tr>");
        }
        sb.AppendLine("</table>");
        ...
        foreach (var role in data.Roles)
        {
            ...
            sb.AppendLine("<table>");
            sb.AppendLine("<tr><th>Display Name</th><th>UPN</th><th>Assignment</th><th>MFA</th><th>Account</th></tr>");
            foreach (var m in role.Members.OrderBy(m => m.PrincipalType).ThenBy(m => m.AssignmentType).ThenBy(m => m.DisplayName))
            {
                var isSp       = m.PrincipalType == "ServicePrincipal";
                var mfaClass   = isSp ? "" : m.IsMfaRegistered ? "good" : "critical";
                var mfaText    = isSp ? "N/A" : m.IsMfaRegistered ? "Yes" : "No";
                var acctClass  = m.AccountEnabled ? "" : "warning";
                var assignClass = m.AssignmentType == "Eligible" ? "warning" : "";
                var rowStyle   = isSp ? " style='color:#6B7280;font-style:italic'" : "";
                sb.AppendLine($"<tr{rowStyle}>" +
                    $"<td>{m.DisplayName ?? "-"}</td>" +
                    $"<td style='font-size:11px'>{(isSp ? "App / Service Principal" : m.UserPrincipalName ?? "-")}</td>" +
                    $"<td class='{assignClass}'>{m.AssignmentType}</td>" +
                    $"<td class='{mfaClass}'>{mfaText}</td>" +
...
public class AdminRoleMember
{
    ...
    public bool AccountEnabled { get; set; }
    public bool IsMfaRegistered { get; set; }
    /// <summary>"Active", "Eligible", or "Active + Eligible"</summary>
